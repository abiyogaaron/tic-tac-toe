...
function App() {
  ...
  const calculateGameStatus = (results: TResults, isComputer: boolean) => {
    ...
    if (isGameOver) {
      scores.firstPlayer = isFirstPlayerWin ? scores.firstPlayer + 1 : scores.firstPlayer;
      scores.secondPlayer = isSecondPlayerWin ? scores.secondPlayer + 1 : scores.secondPlayer;
  ...
  const getLines = (
    ...
    if (rowIdx < 0 || colIdx < 0 || rowIdx > updatedBoards.length - 1 || colIdx > updatedBoards[rowIdx].length - 1) {
      return initialRes; 
    }
    ...
    if (!move) {
      ...
      } else if (move === EMove.DIAGONAL_BOTTOM_LEFT || move === EMove.DIAGONAL_TOP_RIGHT) {
        initialRes[EResult.DIAGONAL_LEFT] = initialRes[EResult.DIAGONAL_LEFT] + 1;
      } else if (move === EMove.DIAGONAL_BOTTOM_RIGHT || move === EMove.DIAGONAL_TOP_LEFT) {
        initialRes[EResult.DIAGONAL_RIGHT] = initialRes[EResult.DIAGONAL_RIGHT] + 1;
      }
    };
    return initialRes;
  };
  ...
  const AiBestMove = useCallback((
    ...
    const getWinner = (
      ...
      const result = getLines(rowIdx, colIdx, boards[rowIdx][colIdx], boards, initialRes);
    ...
    const miniMax = (
      ...
      for (let i=0; i<boards.length; i++) {
        for (let j=0; j<boards[i].length; j++) {
          ...
          boards[i][j] = player;
          const score = miniMax(deepCopy(boards), i, j, !isMax, nextDepth).score;
          boards[i][j] = '';
  ...
  const clickBoard = (rowIdx: number, colIdx: number) => {
    ...
    if (isComputer) {
      ...
      if (i !== rowIdx || j !== colIdx) {
        ...
        const result = getLines(i, j, newBoards[i][j], newBoards, {...initialRes});
    ...
    boardFilledCtr.current = boardFilledCtr.current + 1;
    const result = getLines(rowIdx, colIdx, newBoards[rowIdx][colIdx], newBoards, {...initialRes});
    calculateGameStatus(result, false);
  }

  const generateCell = useCallback((row: Array<string>, rowIdx: number): ReactNode => {
    return row.map((cell, idx) => {
      let content: ReactNode;
      if (cell === 'X') {
        content = <img src={close} width="32" height="32" alt="cross-icon"></img>
      } else if (cell === 'O') {
        content = <img src={circle} width="32" height="32" alt="circle-icon"></img>
      } else if (cell === "") {
        content = null;
      }
      ...
      <div className="container">
        ...
        <div className="row">
          ...
          <div className="block-info">
            ...
            <p>
              {state.isComputer ? state.scores.computer : state.scores.secondPlayer}
            </p>
          </div>
        </div>
        <div className="row">
          <button 
            ...
            onClick={() => {
              dispatch({type: EAppAction.SET_BOARDS, payload: INITIAL_BOARDS });
          ...
          <button
            ...
            disabled={!state.isGameOver}
            onClick={() => {
              dispatch({type: EAppAction.SET_IS_COMPUTER, payload: !state.isComputer });
            }}
          >
